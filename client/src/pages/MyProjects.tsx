      ...
      {loading ? (
        ...
        <div className='py-10 min-h-[80vh]' >
          <div className='flex items-center justify-between mb-12' >
            ...
            <button onClick={() => navigate('/')} className='bg-linear-to-br from-indigo-500 to-indigo-600 text-white py-1 sm:py-2 px-3 sm:px-6 rounded flex items-center gap-2 hover:opacity-90 active-scale-95 transition-all ' >
              <PlusIcon size={18} />
              Create New
            </button>
          </div>
          <div className='flex flex-wrap gap-3.5' >
            {projects.map((project) => (
              <div key={project.id} onClick={() => navigate(`/project/${project.id}`)} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300' >
                  <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800' >
                    {project.current_code ? (
                      ...
                      <div className='flex items-center justify-center h-full' >
                        <h2 className='text-lg font-medium text-gray-400' >No Preview</h2>
                      </div>
                    )}
                </div>
                ...
                <div onClick={(e) => { e.stopPropagation(); /* handle delete */ }} >
                  <TrashIcon size={18} className='absolute top-3 right-3 scale-0 group-hover:scale-100 transition-all bg-white p-1.5 size-7 rounded text-red-500 text-xl cursor-pointer' onClick={() => deleteProject(project.id)} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col items-center justify-center h-[80vh] "  >
          ...
          <button onClick={() => navigate('/')} className='bg-linear-to-br from-indigo-500 to-indigo-600 text-white py-1 sm:py-2 px-3 sm:px-6 rounded flex items-center gap-2 hover:opacity-90 active-scale-95 transition-all mt-6 ' >
            <PlusIcon size={18} />
            Create New
          </button>
        </div>
